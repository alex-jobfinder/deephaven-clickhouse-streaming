...
class CuratedOrderSizes:
    ...
    def __post_init__(self):
        self.dictionary = dict(zip(self.order_sizes, self.fill_prices))

    def get_order_sizes(
        self,
    ):  # FIXME: type hints don't seem to matter for in Class methods
...
def parse_book_curated(book_str: str, max_levels: int = 200):
    ...
    cum_quote_sizes = np.insert(
        np.cumsum(quote_sizes), 0, 0
    )  # add leading [0] here, so we can use np.diff() equivalent logic below
...
def convert_sampled_to_curated(orderbooks_sampled):
    ...
    # join to final table
    l2_curated = bid_curated.natural_join(
        ask_curated, on=["ts_bin", "exchange", "symbol"]
    )
    ...
    # final re-ordering
    l2_curated = l2_curated.move_columns_up([
        "ts_bin",
        "exchange",
        "symbol",
        "best_bid",
...
def get_orderbooks_curated():
    ...
    orderbooks_sampled = orderbooks_blink.last_by([
        "exchange",
        "symbol",
        "ts_cycle",
    ]).move_columns_down(["bid", "ask"])
...
slippage_100k = dx.line(
    ...
    by=[
        "exchange",
        "variable",
    ],  # 'variable' dimension is a plotly oddity, see explanation at https://github.com/deephaven/deephaven-plugins/issues/209
)
